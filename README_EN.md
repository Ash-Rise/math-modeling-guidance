...
To see a compact end-to-end computation first, run the [self-contained canteen example](projects/quickstart-canteen/README.md). It uses only the Python standard library and produces a candidate comparison, result summary, and Markdown report from synthetic inputs.
...
The following projects show the papers and supporting artifacts produced by the workflow. Papers are available as PDF, Word, and Markdown. Each project entry connects to its task summary, consequential decisions, code, results, scripts, and tests.
