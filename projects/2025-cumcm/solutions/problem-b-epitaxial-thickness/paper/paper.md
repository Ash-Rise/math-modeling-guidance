...
红外反射条纹首先约束光学厚度，折射率色散、测量标定及多次反射共同影响几何厚度的反演。针对两组双角度实测光谱，本文建立空气—外延层—衬底模型，以有来源的材料色散为参考，联合估计同片厚度和有效载流子响应，并将反射率增益、偏移作为测量干扰参数。
...
针对问题3，对层内往返振幅作几何级数求和，区分多光束产生条件与可观测条件。硅数据的多光束厚度为 **3.386838 μm**，残差由双光束的 **4.36318** 降至 **1.13271 个百分点**，基于各自全数据候选初始化后的分块重拟合，其同谱留出误差由 **5.00746** 降至 **1.32890 个百分点**，支持该条件下的多光束描述，但不是严格盲留出证据。碳化硅多光束校正后为 **7.402901 μm**，厚度变化不足 **0.001 μm**，小于分角度与参考尺度变化的影响。

独立传输矩阵和直接往返求和验证了反射公式；原采样与隔点拟合的厚度差均小于 **0.0001 μm**。但残差仍有相关结构，且实测数据不能独立确认晶型和绝对标定。因此，实际结论保留为碳化硅约 **7.40 μm**、硅约 **3.39 μm**；已检验假设变化对应的厚度范围分别约为 **7.370—7.530 μm** 和 **3.375—3.489 μm**，这些范围不是置信区间或绝对误差界。
...
同一晶圆片的外延层厚度不会随入射角改变，但层内折射角、界面反射系数和传播相位都会变化。因此，两条光谱应共享厚度与材料参数，而分别使用题面给定的10°和15°入射角。问题1确定首个层内往返产生的双光束模型；问题2将该模型用于碳化硅。问题3新增的物理过程是第二次及以后的层内往返，须在保留原有材料和测量模型的条件下检验其影响，不能把折射率模型变化造成的差异归因于多光束。

本题的主要困难不在于找到若干峰谷，而在于由条纹相位识别几何厚度。题面明确指出折射率随波长和掺杂变化，却未给折射率、掺杂浓度、晶型、偏振和仪器校准。若直接固定一个无来源的常数折射率，厚度可以被迅速算出，其绝对尺度却没有可靠依据。本文采用公开的材料色散参考，并将其适用性视为结论的条件，随后用分角度、参考参数和标定模型对照评估这一条件的影响。
因此，本文把任务分成两个相互约束的层面：正向模型回答“给定厚度、材料响应和测量条件会出现什么光谱”，反演则回答“在这些条件固定到何种程度时，实测光谱支持哪个厚度”。问题1先规定允许保留的光路，问题2在该光路下求参数，问题3再改变往返次数并保持其余比较条件。这样，光学机制的变化与参考尺度的变化不会被合并解释为同一种厚度修正。

若只按相邻峰谷反推厚度，必须预先知道折射率及极值对应的相位级次；若直接拟合全谱，又必须处理材料参数与标定的耦合。本文选择后者作为主求解，以峰距推导和傅里叶定位解释相位信息、提供起点；再用同条件光束对照与假设对照分别回答“遗漏往返项的影响”和“模型闭合的影响”。这两个问题的检验不可互相替代。
...
四表首个反射率均为0，随后快速上升；附件2还有262个反射率超过100%的记录。被动样品的真实反射率不能大于1，但仪器归一化、参考镜误差或其他测量偏差可以使报告值超过100%。仅凭附件不能确定具体原因，故保留原始数值，不截断、不以最高峰强制归一化，在反演中显式加入增益和常量偏移。
...
![](../figures/fig1_measured_spectra.png){width=95%}
...
两角度共用材料参数；每条曲线允许独立的乘性增益和常量偏移。只有这些参数能够解释的缓变测量差异被纳入模型，不用任意高阶背景函数吸收条纹。忽略粗糙度、厚度空间分布和仪器线形的独立参数，因为附件未给相应信息；残差结构用于识别这些简化可能留下的影响，而不是声称它们不存在。
...
这个目标同时使用条纹位置和反射率幅值，但二者的约束作用不同：厚度主要改变传播相位，介电参数也能改变相位并影响界面系数，增益与偏移则改变光谱的纵向尺度。共用厚度和介电参数使两角度共同约束同一晶片；分别设置增益与偏移避免将明显的幅值差异全部强迫归入材料参数。它仍不是对参数唯一性的证明。
...
![](../figures/fig7_search.png){width=95%}
...
图3先展示问题2的双光束预测，并同时列出问题3在相同参考与拟合域下得到的多光束曲线，便于辨认校正发生在哪里。透光段中，两模型大体描述条纹及缓变背景；共振边缘的局部形状仍难以重现，15°曲线尤其明显。两模型预测彼此接近，却一起偏离实测形状，说明“增加往返次数”和“解释全部模型失配”是不同的任务。

![](../figures/fig2_sic_fit_residual.png){width=95%}
...
式(11)说明双角度在理论上有分离$d$与$n$的能力，但两角度接近，右侧的角度差很小。实测中界面相位、色散、基线与校准误差都可能影响相位恢复，不能仅凭有两个角度就认定绝对折射率已被可靠识别。
...
![](../figures/fig6_dispersion.png){width=95%}
...
表3的留出数值来自训练子集重拟合，初始化分别使用双光束、多光束的全数据候选，二者采用相同初始化规则而非相同参数向量。留出块未参与该次拟合，但已参与前序候选形成；这些数值只用于既定初始化规则、参考和标定条件下的同谱分块预测比较，不能作为外部验证、独立样片验证或严格无信息泄漏的泛化证据。硅的多光束模型在该检查中仍保持明显优势；同样数量的自由参数也排除了单纯“增加参数数量”对本次比较的解释。图5进一步展示了差异发生在何处：双光束难以同时匹配低波数强峰和深谷，多光束较好地恢复了这些结构。高波数区仍有有序残差，故其适用性并非无条件成立。

![](../figures/fig3_si_two_vs_multi.png){width=95%}
...
![](../figures/fig4_multibeam_mechanism.png){width=95%}
...
![](../figures/fig5_sensitivity.png){width=95%}
...
![](../figures/fig8_fit_tradeoff.png){width=95%}
...
在固定材料尺度的已检验变化中，SiC厚度约7.370—7.441 μm、Si约3.375—3.402 μm；计入自由尺度对照后，上端分别扩展至约7.530和3.489 μm。这些只是所检验情景的包络，不覆盖全部可能晶型、仪器误差或层结构，更不是绝对误差上界。它们支持把实用结论写成约7.40 μm和约3.39 μm，并明确参考假设。
...
进一步提升绝对测厚可信度，最有价值的是确认晶型与晶向、提供参考镜校准和仪器线形，或增加独立的厚度/折射率测量。相比之下，在当前缺失信息不变时继续收紧优化容差，难以解决主导的尺度不确定性。未建模的厚度非均匀性、各向异性和本征吸收可能贡献残差，现有结果不应被用于对这些因素作唯一诊断。
...
对硅数据，完整往返模型在相同参数数量、频段和标定条件下明显改善强条纹峰形，并在全数据候选初始化后的分块重拟合中保持同谱预测优势（非严格盲留出），得到厚度3.386838 μm。因此，本文在给定参考假设下采用多光束模型解释附件3、4；但实验相干性、光斑重叠和仪器分辨率未被单独测量，不能宣称所有必要条件已由强度数据严格证明。
