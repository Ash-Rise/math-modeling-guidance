...
针对来袭导弹、无人机与固定目标之间的烟幕干扰弹投放问题，本文建立“确定性三维运动学—完整目标视线几何—时间区间测度”模型。真目标按半径 7 m、高 10 m 的有限圆柱建模；逐时刻计算每条导弹—目标表面视线到有效云团的最近距离，再以全部视线中的最大值判定联合完整遮蔽。问题 5 进一步按导弹指派云团，形成逐目标计分口径。
...
问题 5 保留逐弹单一导弹指派，并在同一指派组内计算云团联合效果。从 31.1752 s 的初始可行方案出发，联合搜索共享航迹、时序和指派，最终使用 11 弹，总目标达到 34.7823 s；M1、M2、M3 分别获得 21.5479、8.0909、5.1435 s。表面与时间加密、约束复算支持这些数值，方案定位为记录搜索设置下的 best-found 可行解。
...
五个子问题共用导弹、无人机和烟幕运动学、完整目标视线判据及时间集合测度。问题 1 给定 FY1 的全部动作，是公共评价模型的基础检验；问题 2 在单弹场景下新增航向、速度和时序优化。问题 3 与问题 4 从不同方向扩展问题 2：前者协调共享航迹上的三组投放时序，后者协调三条独立航迹的可达窗口。问题 5 同时引入多机多弹、实际用弹数量和逐弹指派，整体效果由共享航迹、时序和云团互补共同决定。问题 3～5 的逐弹策略分别对应附件 result1.xlsx、result2.xlsx 和 result3.xlsx；正文同时报告空间联合判据得到的整体时长。

## 二、问题分析

导弹和无人机的位置由少量连续变量决定，烟幕弹起爆点又由投放时刻、起爆延时和无人机运动共同确定。因此，核心任务是在可达约束下安排云团的空间位置和有效时段，使其持续位于来袭导弹与真目标之间。

有限尺寸目标带来两个直接影响：完整遮蔽要求所有导弹—圆柱表面视线均被截断，同一时刻的不同视线又可以由不同云团承担。模型因此先计算当前有效云团的空间联合覆盖，再累计完整遮蔽时间；逐弹独立区间则用于解释单弹贡献。
...
当 $\rho_j(t;\mathbf c_{ik}(t))\le R$ 时，烟幕球同时截断导弹到完整目标表面的全部视线，因而记该时刻对 $M_j$ 有效。$\rho_j$ 的几何意义是完成整目标遮蔽所需的最小球半径。

上述距离由线段参数 $\lambda\in[0,1]$ 上的平方距离最小化得到。投影系数经过截断后，判据只计算实际导弹—目标线段；外层最大值则锁定最难遮断的目标表面视线。这样，半径 7 m 和高度 10 m 所形成的视线束宽度直接进入评价。目标中心视线仅在候选构造阶段提供几何引导，正式计时始终使用完整目标判据。
...
当 $A_j(t)$ 为空时，记该时刻无遮挡。内层最小值为每条视线选取距离最近的有效云团，外层最大值再检查其中最不利的一条。因此，$H_j(t)\le10$ 表示每条视线都至少与一团烟幕相交；云团的半径和浓度在这一过程中保持各自的物理含义。
...
式中不等式表明，单团独立完整遮蔽一定进入联合遮蔽集合，而多团还可能通过空间互补增加有效时段。以一个服务 M1 的双云团对照为例：FY4 云团在 17.457440 s 于 $(12380.504797,19.853045,1249.237627)$ m 起爆，FY5 云团在 15.087720 s 于 $(12894.727032,27.015719,1295.784493)$ m 起爆，二者均以 3 m/s 下沉。该构型只用于解释判据。$t=19.05$ s 时，159,840 个表面点给出的单团所需半径分别为 10.304731 m 和 10.237151 m，而联合所需半径为 9.336984 m：两团分别覆盖不同的困难视线后，10 m 半径即可完成整目标遮蔽。

![图1  双云团空间联合完整遮蔽机制](../figures/fig_joint_shielding_mechanism.png){width=100%}

图 1 把完整三维判据的分类结果投影到目标表面的 $y$-$z$ 平面。云团 A 单独作用时留下低位北侧缺口，云团 B 留下高位南侧缺口；二者联合后缺口互补，未覆盖视线数降为 0。投影承担机制解释，正式结论仍由三维距离计算和下述连续时间保守界共同支撑。

为确认这一互补持续正的时间长度，取两团独立完整区间之间的空档 $I=[18.998120870,19.113722246]$ s。两团在整个 $I$ 内均有效。时间上取 117 个等距点，圆柱表面取 720 个方位角、61 个侧面高度层和每底面 51 个半径层，得到采样最大联合半径 $\widehat H_{\max}\approx9.374946$ m。点到线段距离关于目标端点具有 1-Lipschitz 性质，取最小和最大后该性质仍成立，因此最近表面采样点带来的覆盖误差满足
...
单团采样最大值同时给出连续表面最大值的下界。在 $[19.0495,19.0505]$ s 内，较有利的一团仍需至少 $10.237151-303\times0.0005=10.085651$ m 才能独立完整遮蔽。于是，这一正时长区间内只有联合判据成立，证明空间互补是独立区间并集无法表达的新增机制。该保守界用于确认这一局部机制；后续正式方案仍分别进行整体数值复核。

记 $\mathcal J_j=\{t:H_j(t)\le10\}$，并令 $L_j=\mu(\mathcal J_j)$。Q2～Q4 的目标为 $L_1$，Q5 的目标为 $L_1+L_2+L_3$。同一导弹同一时刻计一次，不同导弹则分别累计，因此 Q5 指标表示总威胁—时间；三弹同时遮蔽时长和各导弹间的均衡程度另行报告。
...
联合边界随云团—视线接触关系变化，并在起爆、寿命终止等事件处出现集合切换，因此采用无梯度搜索。问题 2 先以几何参数构造可行起点，再用差分进化^[2]^和 Nelder–Mead^[3]^细化；问题 3～5 直接评价联合时长，使候选得分同时反映已有时间覆盖和视线覆盖下的边际收益。

搜索阶段采用 312 个表面点和 0.12 s 时间网格，以过零点线性插值快速估计时长；种群先围绕已有可行方案生成，再扩大航迹扰动，每轮差分进化使用 28 个个体、最多 24 代。进入候选集后，改用 2250 个表面点和端点求根统一比较。局部阶段以 0.03 s 网格辅助 Nelder–Mead，每机最多 140 次迭代，更新只有在加密复算后才被接受。粗网格负责探索，加密评价负责正式比较。

Q3 搜索共享航迹与三弹时序的八个变量；Q4 联合搜索三机的十二个变量，再逐机细化；Q5 则固定其余机组，交替更新当前机的共享航迹、时序和有限指派。正式复算使用更密表面与时间网格，并按云团生效、失效事件分段，在集合固定的连续段内用 Brent 法细化端点。差分进化阶段将首弹投放时刻控制在 45 s 内、相邻投放的额外间隔控制在 20 s 内，以限定探索规模；局部细化仍按题目物理约束判断可行。最终方案据此定位为已记录搜索范围内的 best-found 解。

每次目标评价先由航迹和投放时序得到各云团的起爆点、有效时间域与服务导弹，再用起爆和失效事件切分时间轴。每个分段内云团集合固定，可连续计算 $H_j(t)-10$；符号变化定位遮蔽边界，端点细化后合并相邻区间并求长度。事件分段把集合突变与连续接触方程分开处理。种群搜索探索空间—时间组合，局部细化改善当前候选，最终加密评价则抑制采样误差对排序的影响。
...
20 s 给出云团寿命上限，实际有效时长还由视线几何决定。云团在 $b=5.1$ s 形成后，完整视线束于 8.056445 s 进入 10 m 覆盖范围；随着导弹前进和云团下沉，最不利视线在 9.448088 s 离开。两次接触之间的 1.391643 s 才是本方案的完整遮蔽时长。
...
满足 $70\le v\le140$、$0\le\widetilde e_z\le f^0_{1,z}$、$\delta\le b$ 且 $b+s<T_1$ 的候选可以由 FY1 实现。引导搜索取 $b\in[0.3,3.0]$ s、$s\in[0,6]$ s、$\lambda\in[0.85,0.9999]$，重点探索早期且靠近导弹的云团位置；随后回到 $\theta,v,b,\delta$ 四个直接变量做局部细化，从而解除中心线对齐这一构造限制。该两阶段设计提高了可行候选密度，其证据边界仍是有限搜索域。
...
该方案的速度达到题面允许上界。为判断这一边界解是否由局部数值波动造成，固定表 3 的其余参数，在 130、134、136、138、139、139.5、139.9、140 m/s 八个速度点复算，时长由 4.583689 s 单调增至 4.588055 s。因而，在当前几何配置附近，提高速度仍有小幅正收益，140 m/s 的取值与局部响应一致。该切片固定了其他变量，描述的是当前方案的局部机制，而不是各速度约束下重新优化后的最优值函数。

另作 5 组多起点搜索和 7 个速度上限情景。在 0.1 s 时间网格、168 个表面点下，多组候选均得到约 4.60 s，说明粗评价已不足以排序；改用 0.01 s 网格、3,780 个表面点后，最有竞争力的对照为 4.59 s，端点精算值为 4.580870 s，仍低于表 3 的 4.588055 s。400 次可行局部扰动也未发现更长候选。由此，表 3 方案通过了同判据加密比较和局部挑战，作为当前搜索设置下的 best-found 解进入后续报告。
...
本题的增益来自联合目标下对共享航迹和三组时序的重新配置。最终方案的联合时段恰好等于三段独立区间的并集，表明其主要机制是时间接力；统一使用联合判据，则保证其他可能出现空间互补的候选也在同一口径下参与比较。
...
单弹独立时长不足以直接排序拼接：同机多弹共享航向和速度，一次航迹调整会同步改变该机全部投放点与起爆点；投放间隔形成时序硬耦合；指派变化又同时改变两个导弹组的既有时间覆盖和视线覆盖。求解器因此交替更新当前机的连续航迹、时序和有限指派，并在每次更新后复算整套方案，使每个接受的改动都对应正式联合目标的真实增益。
...
同一方案若直接相加逐弹独立完整时长，得到 34.655856 s；在同一导弹内先取时间并集后为 33.726381 s，其中消除了约 0.929475 s 的重复计时。进一步按空间联合判据计算，不同云团对目标视线的互补又增加 **1.055879 s**，最终得到 34.782261 s。这个分解同时揭示了两种相反效应：时间重叠使逐弹和偏高，空间互补使独立区间并集偏低。三枚导弹同时完整遮蔽的 2.459609 s 是同步防护指标；正式目标则累计三个威胁各自的受遮蔽时间。
...
数值证据分为四层：正确性检查核对运动学、约束、指派和联合判据；稳定性检查比较固定动作在表面加密、时间定位与端点细化下的变化；敏感性分析考察输入或执行条件扰动后的响应；搜索证据则来自同一目标下的多起点、候选挑战和局部改进。本文完成了前两层及问题 2 的速度切片，并用有限搜索挑战支持 best-found 定位；风场、定位和引信误差仍需额外数据才能形成系统敏感性结论。
...
本文模型把真目标的半径和高度直接纳入视线束，区分了中心视线遮断与完整可见轮廓遮断。先做云团空间联合、再构造时间集合的评价顺序，统一处理视线互补、时间重叠和多导弹分别计时。运动学公式连接航迹、投放点、起爆点和有效区间，最终方案的速度、非负时序、起爆高度与同机投放间隔均可逐项复算。
...
问题 1、2 给出单云团基线；问题 3 通过共享航迹和三组时序形成连续接力；问题 4 以三条独立航迹覆盖来袭过程的不同阶段；问题 5 再由共享航迹、投放时序和指派共同配置三枚导弹的时间与视线覆盖。问题 3～5 均以多团空间联合完整遮蔽为统一判据，最终数值代表记录搜索设置下经过加密复核的 best-found 可行方案。
